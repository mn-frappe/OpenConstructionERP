...
function renderCoverPage(
  ...
  // "OpenEstimate" wordmark
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(...WHITE);
  doc.text('OpenEstimate', 20, 20);
  ...
  const metaItems: Array<[string, string]> = [
    ...
    ['Positions', String(options.positions.filter((p) => !isSection(p)).length)],
    ['Gross Total', formatCurrency(options.grossTotal, options.currency, locale)],
  ];
  ...
  doc.setFontSize(8);
  doc.setTextColor(...BRAND_MID);
  doc.text('Generated by OpenEstimate — open-source construction estimation platform', 20, pageH - 15);
}
